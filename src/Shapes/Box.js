...
export default function Box(name, position, options) {
  // If there is no scene, we do nothing.
    if(Director.scene instanceof Scene === false) {
      ...
      Director.mesh = CreateBox(name, {width: width, height: height, depth: depth}, Director.scene);
      
      // Assign position to Director.mesh.position.
      Director.mesh.position = new Vector3(x, y, z);
    }
}
